...
function Avatar({ user, size = "sm" }) {
  ...
  return (
    <div className={`${sz} rounded-full flex items-center justify-center font-medium flex-shrink-0 ${getAvatarColor(user?.id)}`}>
      {initials || "?"}
    </div>
  );
}
...
function CompletedBadge() {
  return (
    <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-medium bg-emerald-50 border border-emerald-200 text-emerald-600">
  ...
  return (
    <>
      ...
      <div className="fixed inset-y-0 right-0 z-50 w-full sm:w-[480px] bg-white shadow-2xl border-l border-gray-200 flex flex-col animate-slide-in">
        ...
        {/* Drawer Header - Clean white design */}
        <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100 bg-white">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-emerald-100 flex items-center justify-center">
              <svg className="w-5 h-5 text-emerald-600" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.8}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
            </div>
            <div>
              <span className="text-xs font-mono text-emerald-600 font-semibold bg-emerald-50 px-2 py-0.5 rounded">
                {task.task_number}
              </span>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-6 space-y-6">
          ...
          <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4 flex items-start gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-emerald-700">Task Completed</p>
              <p className="text-xs text-emerald-600/80 mt-0.5">This task has been successfully completed.</p>
            </div>
          </div>
        </div>
...
export default function CompletedTasksPage() {
  ...
  const fetchTasks = useCallback(async () => {
    ...
    try {
      ...
      setTasks((json.data ?? []).filter((t) => t.status === "completed"));
    } catch (err) {
  ...
  const ViewButton = ({ task }) => (
    <button
      ...
      className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium text-emerald-600 bg-emerald-50 border border-emerald-200 hover:bg-emerald-100 transition-colors"
    >
      <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-400 mt-0.5">
                All successfully completed project tasks
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            {!loading && (
              <span className="hidden sm:inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium bg-emerald-100 text-emerald-600">
        ...
        <div className="mb-6">
          <div className="relative max-w-md">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 border border-gray-200 rounded-lg bg-white text-gray-700 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-emerald-400 focus:border-transparent transition"
            />
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3 mb-6">
          <select
            ...
            className="px-3 py-1.5 rounded-lg border border-gray-200 bg-white text-sm text-gray-600 focus:outline-none focus:ring-2 focus:ring-emerald-400"
          >
        ...
        {!loading && !error && filtered.length > 0 && (
          <p className="text-center text-xs text-gray-400 mt-6">
            Showing {filtered.length} of {tasks.length} completed task{tasks.length !== 1 ? "s" : ""}
          </p>
        )}
      </div>
