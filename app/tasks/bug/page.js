...
function BugBadge() {
  return (
    <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-medium bg-amber-100 border border-amber-200 text-amber-700">
      <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2.5}>
        <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v2m0 4h.01m-6.938 4A9 9 0 1118.938 7 9 9 0 015.062 17z" />
      </svg>
      Bug
    </span>
  );
      ...
      <div className="fixed inset-y-0 right-0 z-50 w-full sm:w-[480px] bg-white shadow-2xl border-l border-gray-200 flex flex-col animate-slide-in">
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100 bg-white">
          <div className="flex items-center gap-3">
            ...
            <div>
              <span className="text-xs font-mono text-amber-600 font-semibold bg-amber-50 px-2 py-0.5 rounded">
                {task.task_number}
              </span>
              <p className="text-xs text-gray-400 mt-1">Bug Details</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto px-6 py-6 space-y-6">
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex items-start gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-amber-700">Bug Task</p>
              <p className="text-xs text-amber-600/80 mt-0.5">This task is marked as a bug and requires investigation.</p>
            </div>
          </div>
        </div>
...
export default function BugTasksPage() {
  ...
  const fetchTasks = useCallback(async () => {
    ...
    try {
      ...
      setTasks((json.data ?? []).filter((t) => t.status === "bug"));
    } catch (err) {
  ...
  const filtered = useMemo(() => tasks.filter((t) => {
    ...
    return (
      ...
      (projectFilter  === "All" || t.project?.name === projectFilter)
    );
  ...
  const ViewButton = ({ task }) => (
    <button
      ...
      className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium text-amber-600 bg-amber-50 border border-amber-200 hover:bg-amber-100 transition-colors"
    >
      <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
          <div className="flex items-start gap-4">
            ...
            <div>
              <h1 className="text-2xl sm:text-3xl font-bold text-gray-800">
                Bug <span className="text-amber-500">Tasks</span>
              </h1>
              <p className="text-sm text-gray-400 mt-0.5">
                All reported bug tasks
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            {!loading && (
              <span className="hidden sm:inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium bg-amber-100 text-amber-600">
        ...
        <div className="mb-6">
          <div className="relative max-w-md">
            ...
            <input
              type="text"
              placeholder="Search bug, assignee, project..."
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              className="w-full pl-10 pr-4 py-2 border border-gray-200 rounded-lg bg-white text-gray-700 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-amber-400 focus:border-transparent transition"
            />
          </div>
        </div>
        ...
        {/* Section Header */}
        <div className="mb-4">
          <h2 className="text-lg font-semibold text-gray-700">Bug Tasks</h2>
        </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-200 bg-gray-50">
                  <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">BUG #</th>
                  ...
                  <tr>
                    <td colSpan={7} className="py-16 text-center">
                      <div className="flex flex-col items-center gap-2 text-gray-400">
                        ...
                        <p className="text-sm">No bug tasks found</p>
        ...
        {!loading && !error && filtered.length > 0 && (
          <p className="text-center text-xs text-gray-400 mt-6">
            Showing {filtered.length} of {tasks.length} bug task{tasks.length !== 1 ? "s" : ""}
          </p>
        )}
      </div>
