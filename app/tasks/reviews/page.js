...
function ReviewBadge() {
  return (
    <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-medium bg-indigo-100 border border-indigo-200 text-indigo-600">
      ...
      <div className="fixed inset-y-0 right-0 z-50 w-full sm:w-[480px] bg-white shadow-2xl border-l border-gray-200 flex flex-col animate-slide-in">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-6 space-y-6">
          ...
          <div className="bg-indigo-50 border border-indigo-200 rounded-xl p-4 flex items-start gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-indigo-700">Under Review</p>
              <p className="text-xs text-indigo-600/80 mt-0.5">This task is currently in the review phase.</p>
            </div>
          </div>
        </div>
...
export default function ReviewTasksPage() {
  ...
  const fetchTasks = useCallback(async () => {
    ...
    try {
      ...
      // Filter only Review tasks
      const reviewTasks = (json.data ?? []).filter((t) => t.status === "review");
  ...
  const ViewButton = ({ task }) => (
    <button
      ...
      className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium text-indigo-600 bg-indigo-50 border border-indigo-200 hover:bg-indigo-100 transition-colors"
    >
      <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
          <div className="flex items-start gap-4">
            <div className="w-12 h-12 rounded-xl bg-indigo-100 flex items-center justify-center flex-shrink-0">
              <svg className="w-6 h-6 text-indigo-600" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.8}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
            </div>
            <div>
              <h1 className="text-2xl sm:text-3xl font-bold text-gray-800">
                Review <span className="text-indigo-500">Tasks</span>
              </h1>
              <p className="text-sm text-gray-400 mt-0.5">
                All tasks pending review
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            {!loading && (
              <span className="hidden sm:inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium bg-indigo-100 text-indigo-600">
        ...
        <div className="mb-6">
          <div className="relative max-w-md">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 border border-gray-200 rounded-lg bg-white text-gray-700 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:border-transparent transition"
            />
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3 mb-6">
          <select
            ...
            className="px-3 py-1.5 rounded-lg border border-gray-200 bg-white text-sm text-gray-600 focus:outline-none focus:ring-2 focus:ring-indigo-400"
          >
        ...
        {/* Section Header */}
        <div className="mb-4">
          <h2 className="text-lg font-semibold text-gray-700">Review Tasks</h2>
        </div>
                  ...
                  <tr>
                    <td colSpan={7} className="py-16 text-center">
                      <div className="flex flex-col items-center gap-2 text-gray-400">
                        ...
                        <p className="text-sm">No review tasks found</p>
        ...
        {!loading && !error && filtered.length > 0 && (
          <p className="text-center text-xs text-gray-400 mt-6">
            Showing {filtered.length} of {tasks.length} review task{tasks.length !== 1 ? "s" : ""}
          </p>
        )}
      </div>
