...
function Modal({ title, onClose, children }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center px-4 sm:px-0">
      ...
      <div className="relative w-full sm:max-w-lg bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl p-5 sm:p-8 z-10 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-5 sm:mb-6">
          <h2 className="text-lg sm:text-xl font-bold text-slate-800 tracking-tight">{title}</h2>
          <button
            onClick={onClose}
            className="p-2 -m-2 rounded-lg hover:bg-slate-100 text-slate-400 hover:text-slate-600 transition-colors"
...
function ClaimTypeForm({ initial = EMPTY_FORM, onSubmit, loading }) {
  ...
  const fieldNormal = "border-slate-200 bg-slate-50 hover:border-slate-300 text-slate-800 placeholder:text-slate-300";
  const fieldError = "border-rose-400 bg-rose-50 text-slate-800";
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      {/* Max Amount */}
      <div>
        <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5">Max Amount (₹)</label>
      ...
      <div className="flex flex-col sm:flex-row gap-4 pt-2">
        <div className="flex items-center justify-between sm:justify-start gap-3 bg-slate-50 rounded-xl px-4 py-3 flex-1 border border-slate-100">
          <div>
            <p className="text-sm font-medium text-slate-700">Requires Document</p>
            <p className="text-xs text-slate-400">Receipt / proof mandatory</p>
          </div>
        ...
        <div className="flex items-center justify-between sm:justify-start gap-3 bg-slate-50 rounded-xl px-4 py-3 flex-1 border border-slate-100">
          <div>
            ...
            <p className="text-xs text-slate-400">Available for claims</p>
          </div>
...
function DeleteConfirm({ item, onConfirm, onCancel, loading }) {
  return (
    <div className="text-center space-y-5">
      ...
      <div>
        <p className="text-slate-800 font-semibold text-base sm:text-lg">Delete "{item.name}"?</p>
        <p className="text-slate-500 text-sm mt-2">This action cannot be undone. The claim type will be permanently removed.</p>
      </div>
...
export default function ClaimTypesPage() {
  ...
  return (
   <div className="font-sans min-h-screen" style={{ backgroundColor: '#f9fafb' }}>
      {/* Header */}
      <header className="bg-white border-b border-gray-200 sticky top-0 z-30 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 sm:py-4 flex items-center justify-between gap-3">
          <div className="flex items-center gap-2 sm:gap-3">
            <div className="w-8 h-8 sm:w-9 sm:h-9 bg-orange-500 rounded-xl flex items-center justify-center shadow-md">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 hidden sm:block">Manage reimbursement categories</p>
            </div>
          </div>
          <button
            onClick={() => setModal({ type: "create" })}
            className="flex items-center gap-1.5 sm:gap-2 bg-orange-500 hover:bg-orange-600 active:bg-orange-700 text-white text-sm font-semibold px-3 sm:px-4 py-2 sm:py-2.5 rounded-xl shadow-md transition-colors focus:outline-none focus:ring-2 focus:ring-orange-400 focus:ring-offset-1"
          >
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-5 sm:py-6 space-y-5 sm:space-y-6">
        ...
        <div className="grid grid-cols-3 gap-2.5 sm:gap-4">
          {[
            ...
            <div key={label} className="bg-white rounded-xl sm:rounded-2xl p-3 sm:p-5 border border-gray-200 shadow-sm">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2.5 rounded-xl border border-gray-200 bg-white text-sm text-gray-900 placeholder:text-gray-400 outline-none focus:ring-2 focus:ring-orange-400 focus:border-transparent transition-all"
            />
          </div>
          <div className="flex gap-1.5 bg-white border border-gray-200 rounded-xl p-1 self-start sm:self-auto">
        ...
        {loading ? (
          <div className="flex items-center justify-center py-24">
            <div className="flex flex-col items-center gap-3">
              <Spinner size={36} />
              <p className="text-gray-400 text-sm">Loading claim types…</p>
            </div>
          </div>
        ) : filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 sm:py-24 gap-4">
            ...
            <div className="text-center">
              <p className="text-gray-700 font-semibold">No claim types found</p>
            ...
            {!search && (
              <button
                onClick={() => setModal({ type: "create" })}
                className="flex items-center gap-2 text-orange-600 hover:text-orange-700 text-sm font-semibold border border-orange-200 hover:border-orange-300 bg-orange-50 hover:bg-orange-100 px-4 py-2 rounded-xl transition-colors"
              >
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
                  <path d="M12 5v14M5 12h14" />
                </svg>
          ...
          <>
            {/* Desktop Table */}
            <div className="hidden md:block bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden w-full min-w-0">
                      ...
                      <tr key={ct.id} className="hover:bg-gray-50 transition-colors group">
                        <td className="px-5 py-4">
                          <div className="flex items-center gap-2.5">
                            <div className="w-8 h-8 rounded-lg bg-orange-50 flex items-center justify-center shrink-0 text-orange-500 font-bold text-xs">
                              {ct.name.charAt(0).toUpperCase()}
                            </div>
                            <span className="font-semibold text-gray-900 truncate">{ct.name}</span>
                          </div>
                        </td>
                        <td className="px-5 py-4">
                          <p className="truncate text-gray-500">{ct.description}</p>
                        </td>
                        ...
                        <td className="px-5 py-4">
                          <div className="flex items-center justify-end gap-1.5">
                            <button
                              ...
                              className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-orange-50 hover:bg-orange-100 text-orange-600 text-xs font-semibold transition-colors"
                            >
                            ...
                            <button
                              ...
                              className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-rose-50 hover:bg-rose-100 text-rose-500 text-xs font-semibold transition-colors"
                            >
                              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
                                <polyline points="3 6 5 6 21 6" />
                                <path d="M19 6l-1 14H6L5 6" />
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                      <div className="w-10 h-10 rounded-xl bg-orange-50 flex items-center justify-center shrink-0 text-orange-500 font-bold text-base">
                        {ct.name?.charAt(0).toUpperCase()}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="font-semibold text-sm text-gray-900 leading-tight truncate">{ct.name}</p>
                  ...
                  <div className="flex justify-between text-xs pt-2 border-t border-gray-100">
                    ...
                    <div className="flex flex-col text-right">
                      ...
                      <span className={`font-semibold ${ct.requires_document ? "text-amber-600" : "text-gray-400"}`}>
                  ...
                  <div className="flex flex-col sm:flex-row gap-2 pt-2">
                    <button
                      onClick={() => setModal({ type: "edit", item: ct })}
                      className="w-full flex items-center justify-center gap-2 py-3 rounded-xl bg-orange-50 active:bg-orange-200 text-orange-600 text-sm font-semibold transition"
                    >
                    ...
                    <button
                      onClick={() => setModal({ type: "delete", item: ct })}
                      className="w-full flex items-center justify-center gap-2 py-3 rounded-xl bg-rose-50 active:bg-rose-200 text-rose-500 text-sm font-semibold transition"
                    >
                      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                        <polyline points="3 6 5 6 21 6" />
                        <path d="M19 6l-1 14H6L5 6" />
