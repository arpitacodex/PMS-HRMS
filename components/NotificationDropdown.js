...
// ── Cleans up messages that have "undefined"/"null" baked in from the backend ──
function cleanMessage(msg) {
  if (!msg) return msg;
  return msg
    .replace(/\bundefined\b/gi, "")   // remove stray "undefined"
    .replace(/\bnull\b/gi, "")        // remove stray "null"
    .replace(/\s+/g, " ")             // collapse leftover double spaces
    .trim();
}
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="text-[13px] leading-snug font-semibold" style={{ color: "var(--text-primary)" }}>
                  {cleanMessage(notice.message) || notice.title}
                </p>
