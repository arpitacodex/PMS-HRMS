...
// ─────────────────────────────────────────────────────────────────────────────
// Exports:
//   <EmojiPickerPanel>    — shared grid/search/category UI (no trigger button)
//                           used by BOTH the composer button and the
//                           "more reactions" flow on a message bubble
//   <EmojiPickerButton>   — 😊 button that opens EmojiPickerPanel
...
// ── EmojiPickerPanel ──────────────────────────────────────────────────────────
// Shared grid/search/category UI, no trigger button attached.
// Used by EmojiPickerButton (composer) AND the "more reactions" button on a
// message bubble, so both open the exact same picker.
export function EmojiPickerPanel({ onSelect, onClose, className = "" }) {
  ...
  const handleSelect = useCallback((emoji) => {
    onSelect?.(emoji);
    // Keep open so user can pick multiple (matches previous composer behavior)
  }, [onSelect]);
  ...
  return (
    <div
      ...
      className={`bg-white dark:bg-gray-900 rounded-2xl shadow-2xl
        ...
        w-[320px] flex flex-col overflow-hidden ${className}`}
    ...
    >
      ...
      {/* Emoji grid */}
      <div className="overflow-y-auto px-2 pb-3" style={{ maxHeight: 220 }}>
...
// ── EmojiPickerButton ─────────────────────────────────────────────────────────
// 😊 button (composer) — opens EmojiPickerPanel below-left of the button.
...
export function ReactionBar({ messageId, isMine, onReact, onMoreReactions }) {
  ...
  return (
    // Floats ABOVE the bubble (-top-10) anchored to the same side as the
    ...
    >
      ...
      {/* More reactions button — opens the full EmojiPickerPanel */}
      <button
...
export function useReactions(currentUserId) {
  ...
  // ── Wipe reactions for a deleted message ─────────────────────────────
